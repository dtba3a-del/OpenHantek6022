...
void GlScope::updateXY( const std::array< const XYRecorder *, DsoSettingsScope::maxXYCurves > &recorders,
    ...
    for ( int i = 0; i < DsoSettingsScope::maxXYCurves; ++i ) {
        ...
        if ( xCh < scope->voltage.size() ) {
            gainX = scope->physicalGain( xCh );
            offsetX = scope->voltage[ xCh ].offset;
        }
        if ( yCh < scope->voltage.size() ) {
            gainY = scope->physicalGain( yCh );
    ...
    if ( scope->horizontal.format == Dso::GraphFormat::XY && scope->horizontal.xyContinuous ) {
        ...
        for ( int i = 0; i < DsoSettingsScope::maxXYCurves; ++i ) {
            if ( xyPointCount[ i ] <= 1 )
                continue;
            anyCurveDrawn = true;
            // Color each curve from the Y-channel color (TZ §7.1.3).
            const XYCurveConfig &cfg = m_xyCurveConfigs[ i ];
            QColor color = ( cfg.yChannel < view->colors->voltage.size() ) ? view->colors->voltage[ cfg.yChannel ]
                                                                          : view->colors->voltage[ 0 ];
            m_program->setUniformValue( colorLocation, color );
            m_vaoXY[ i ].bind();
            gl->glLineWidth( 2 );
            gl->glDrawArrays( GL_LINE_STRIP, 0, xyPointCount[ i ] );
            m_vaoXY[ i ].release();
...
void GlScope::drawXYLegend() {
    ...
    for ( int i = 0; i < DsoSettingsScope::maxXYCurves; ++i ) {
        ...
        const QString xName = scope->voltage[ cfg.xChannel ].name;
        const QString yName = scope->voltage[ cfg.yChannel ].name;
        const QString xUnit = scope->voltage[ cfg.xChannel ].ctpuUnit.isEmpty()
                                  ? QStringLiteral( "V" )
                                  : scope->voltage[ cfg.xChannel ].ctpuUnit;
        const QString yUnit = scope->voltage[ cfg.yChannel ].ctpuUnit.isEmpty()
                                  ? QStringLiteral( "V" )
                                  : scope->voltage[ cfg.yChannel ].ctpuUnit;
        const double xGain = scope->physicalGain( cfg.xChannel );
        const double yGain = scope->physicalGain( cfg.yChannel );

        const QString line = QStringLiteral( "%1(x=%2/дел; y=%3/дел)" )
                                 .arg( yName )
