...
/// \brief Converts double to string with an arbitrary user-defined unit (CtPU).
/// Picks an SI prefix (µ, m, k, M, G) based on the magnitude of `value` and
/// appends `unit` verbatim. Used for physical-unit displays (°C, kPa, A, W, …)
/// introduced by CtPU (TZ §3.5.3). When `unit` is empty or "V", behaviour
/// matches the UNIT_VOLTS case of the enum overload above.
/// \param value The value in prefixless units.
/// \param unit The user-defined unit string (1–8 chars, e.g. "°C", "A", "W").
/// \param precision Significant digits, 0 for integer, -1 for auto.
/// \return String like "1.23 k°C" or "45.6 µA".
QString valueToString( double value, const QString &unit, int precision = -1 );
