...
QString valueToString( double value, const QString &unit, int precision ) {
    // TZ §3.5.3 — generic physical-unit formatter.
    // Picks an SI prefix based on the magnitude of `value`, formats with the
    // requested precision, then appends the user-supplied unit verbatim.
    // Empty unit falls back to plain number (matches UNIT_NONE behaviour).
    char format = ( precision < 0 ) ? 'g' : 'f';

    if ( unit.isEmpty() ) {
        return QString::number( value, format,
                                ( precision <= 0 )
                                    ? precision
                                    : qBound( 0, precision - 1 - int( floor( log10( fabs( value ) ) ) ), precision ) );
    }

    if ( !bool( fabs( value ) ) ) {
        return QApplication::tr( "%1 %2" ).arg( 0 ).arg( unit );
    }
    ...
    int scaledLog = ( scaled != 0.0 ) ? int( floor( log10( fabs( scaled ) ) ) ) : 0;
    int decimals = ( precision <= 0 ) ? precision : qBound( 0, precision - 1 - scaledLog, precision );

    return QApplication::tr( "%1 %2%3" ).arg( scaled, 0, format, decimals ).arg( prefix ).arg( unit );
}
