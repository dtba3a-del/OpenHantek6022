...
QStringList VoltageDock::gainStringsForChannel( ChannelID channel, double attnValue ) const {
    ...
    const QString unit = v.ctpuUnit.isEmpty() ? QStringLiteral( "V" ) : v.ctpuUnit;
    if ( channel < spec->channels ) {
        for ( auto gainStep : spec->gain )
            result << valueToString( gainStep.Vdiv * attnValue * v.ctpuK, unit, 0 );
    } else {
        for ( double mathGainStep : scope->mathGainSteps )
            result << valueToString( mathGainStep * v.ctpuK, unit, 0 );
    }
    return result;
}
