...
void DsoSettings::load() {
    ...
    for ( ChannelID channel = 0; channel < scope.voltage.size(); ++channel ) {
        ...
        // -- TZ §3.6 — CtPU fields (backward-compatible: missing keys keep defaults)
        if ( storeSettings->contains( "ctpuMode" ) )
            scope.voltage[ channel ].ctpuMode = CtPU::Mode( storeSettings->value( "ctpuMode" ).toInt() );
        if ( storeSettings->contains( "ctpuUnit" ) )
            scope.voltage[ channel ].ctpuUnit = storeSettings->value( "ctpuUnit" ).toString();
        if ( storeSettings->contains( "ctpuK" ) )
            scope.voltage[ channel ].ctpuK = storeSettings->value( "ctpuK" ).toDouble();
        if ( storeSettings->contains( "ctpuB" ) )
            scope.voltage[ channel ].ctpuB = storeSettings->value( "ctpuB" ).toDouble();
...
void DsoSettings::save() {
    ...
    for ( ChannelID channel = 0; channel < scope.voltage.size(); ++channel ) {
        ...
        // -- TZ §3.6 — persist CtPU fields
        storeSettings->setValue( "ctpuMode", int( scope.voltage[ channel ].ctpuMode ) );
        storeSettings->setValue( "ctpuUnit", scope.voltage[ channel ].ctpuUnit );
        storeSettings->setValue( "ctpuK", scope.voltage[ channel ].ctpuK );
        storeSettings->setValue( "ctpuB", scope.voltage[ channel ].ctpuB );
